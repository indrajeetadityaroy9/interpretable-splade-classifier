...
from lexical_sae.core.types import circuit_mask_by_mass
from lexical_sae.evaluation.collect import collect_sparse_and_attributions
from lexical_sae.core.constants import CIRCUIT_MASS_FRACTION, EPSILON
from lexical_sae.runtime import unwrap_compiled
...
def compute_cpr(
    ...
    return {
        "cpr": circuit_accuracy / max(full_accuracy, EPSILON),
...
def compute_cmd(
    ...
    def _cpr_at_mass(mf: float) -> float:
        ...
        with torch.inference_mode():
            logits = _model.classifier_logits_only(circuit_sparse)
        acc = (logits.argmax(dim=-1) == labels_t).float().mean().item()
        return acc / max(full_accuracy, EPSILON)
