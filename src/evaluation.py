...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, threshold: float = 1e-2) -> Tuple[float, float, float]:
    """
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            y = batch['label'] # Keep labels on CPU for sklearn metrics if possible, or move later
