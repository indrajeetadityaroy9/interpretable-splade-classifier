...
class LEACEWrappedModel(nn.Module):
    ...
    def classify(self, sparse_sequence, attention_mask) -> CircuitState:
        ...
        sparse_vector = _orig.to_pooled(sparse_sequence, attention_mask)
        # Apply LEACE projection
        erased = self.eraser(sparse_vector.float()).to(sparse_vector.dtype)
        logits, W_eff, b_eff = _orig.classifier_forward(erased)
    ...
    def classifier_logits_only(self, sparse_vector):
        erased = self.eraser(sparse_vector.float()).to(sparse_vector.dtype)
        return unwrap_compiled(self.model).classifier_logits_only(erased)

    def classifier_forward(self, sparse_vector):
        erased = self.eraser(sparse_vector.float()).to(sparse_vector.dtype)
        return unwrap_compiled(self.model).classifier_forward(erased)

    def classifier_parameters(self):
        return unwrap_compiled(self.model).classifier_parameters()

    @staticmethod
    def to_pooled(sparse_sequence, attention_mask):
